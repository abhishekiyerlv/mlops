...
| staging     | The `staging` deployment target is part of the CD pipeline. Latest https://adb-2376768479807879.19.azuredatabricks.net/explore/data/dev?o=2376768479807879 content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-mlops-model | /staging-mlops-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest dev content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-mlops-model    | /prod-mlops-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the https://adb-2376768479807879.19.azuredatabricks.net/explore/data/dev?o=2376768479807879 branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-mlops-model    | /test-mlops-experiment    |
...
You can open a PR (pull request) to modify ML code or the resource config against https://adb-2376768479807879.19.azuredatabricks.net/explore/data/dev?o=2376768479807879 branch.
...
Upon merging a PR to the https://adb-2376768479807879.19.azuredatabricks.net/explore/data/dev?o=2376768479807879 branch, the https://adb-2376768479807879.19.azuredatabricks.net/explore/data/dev?o=2376768479807879 branch content will be deployed to the staging workspace with `staging` environment resource configurations.
...
```
Open a pull request to merge the pushed branch into the `https://adb-2376768479807879.19.azuredatabricks.net/explore/data/dev?o=2376768479807879` branch.
...
Once CI passes, merge the PR into the `https://adb-2376768479807879.19.azuredatabricks.net/explore/data/dev?o=2376768479807879` branch. This will deploy an initial set of Databricks resources to the staging workspace.
resources will be deployed to the prod workspace on pushing code to the `dev` branch.
...
After setting up CI/CD, PRs against the https://adb-2376768479807879.19.azuredatabricks.net/explore/data/dev?o=2376768479807879 branch will trigger CI workflows to run unit tests, integration test and resource validation.
...
After merging a PR to the https://adb-2376768479807879.19.azuredatabricks.net/explore/data/dev?o=2376768479807879 branch, continuous deployment automation will deploy the `staging` resources to the staging workspace.

When you about to cut a release, you can create and merge a PR to merge changes from https://adb-2376768479807879.19.azuredatabricks.net/explore/data/dev?o=2376768479807879 to dev. Continuous deployment automation will deploy `prod` resources to the prod workspace.
